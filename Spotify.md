...
Swagger is a set of open-source tools that allow anyone to visualize and interact with your API's resources without having any of the implementation logic in place. The java library springdoc-openapi helps automating the generation of API documentation using spring boot projects.

In order to use it, we add the following dependencies to Maven
...
Also as a help for manipulating Swagger documentations and its output, we use swagger annotations
...
```
To find out specific annotations and their usages click the [link](https://github.com/swagger-api/swagger-core/wiki/Annotations-1.5.X)

With our dependencies now set, in order to use swagger we need an OpenAPIConfig class.

![OpenAPIConfig class](media/Spotify/openApiConfig.png)

Now with this configurations we are ready to start using Swagger

When the project is running, we go [here](http://localhost:8080/spotify/v1/swagger-ui/index.html?configUrl=/spotify/v1/v3/api-docs/swagger-config#/) to open Swagger. We will see a page with all our created end-points and some autogenerated ones. 

Each end-point if formed by diferent parts. For requesting information they need to realize their function, they use parameters or request body.
In the following example, an id is needed so its requested to the user via parameter.

![Song id request](media/Spotify/parameters.png)

Here we need to update a song and the information its requested via json with a request body.

![update song request](media/Spotify/requestBody.png)

Below the requests we find the "Response" camp, where all the posible responses and their outputs are listed.

![responses get genre](media/Spotify/responses.png)

After executing the endpoint we will see the response given by Swagger and its output on top.

![response 200 get genre](media/Spotify/correctResponse.png)
